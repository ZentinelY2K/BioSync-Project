...
def main():
    ...
    while True:
       ...
       # }
        ...
        def thumb_up():
           ...
           # PWM_values_thumb_up = round(ratio*120,2)
            ...
            if ratio is None or distance_ThumbTip_to_IndexKnuckle is None or distance_palm_to_middleKnuckle is None:
                return None
            elif ratio >= 1.10 and distance_ThumbTip_to_IndexKnuckle >= 150:
            #else:
               thumb_up = True
        ...
        def set_volume(up=False,down=False):
            if up:
                sub.run(["xdotool", "key", "XF86AudioRaiseVolume"])
            elif down:
                sub.run(["xdotool", "key", "XF86AudioLowerVolume"])
        def thumb_down():
            global thumb_down
            center_y_FthumbTip = landmarkList[fingers["Thumb"]["Tip"]][2]
        ...
        def shotGun_leftMost():
           ...
           # ratio_thumb_tip_ring = non_zero_div(tip_of_thumb_y, middle_of_ring)
            ...
            elif ratio_shotgun_palm <= -0.20 and ratio_shotGunPalm_indexKnuckle <=  -0.20 and palm_x <= 960 and ratio_index_tip_middle_tip >= 2.00 and ratio_thumb_tip_to_middle >= 1.15:
                shotgun_left = True
        ...
        def mouse_logic():
            ...
            index_finger_tracing_y = landmarkList[fingers['Index']['Tip']][2]
            index_finger_tracing_x = landmarkList[fingers['Index']['Tip']][1]
            
            knuckle_index = landmarkList[knuckles["index_knuckles"]][2]

            index_finger_tracing_y = landmarkList[fingers['Index']['Tip']][2]
            Distance_between_index_and_its_knuckle = get_distance_pixels(index_finger_tracing_y,knuckle_index)

            print(Distance_between_index_and_its_knuckle)

            if (previous_x == index_finger_tracing_x and previous_y == index_finger_tracing_y ) or (previous_x - index_finger_tracing_x <= 10 and previous_y - index_finger_tracing_y <= 10):
                pass
            else:
                pyautogui.moveTo(index_finger_tracing_x,index_finger_tracing_y)
            
            if Distance_between_index_and_its_knuckle >= 130 and Distance_between_index_and_its_knuckle < 150:
                tm.sleep(0.5)
            elif Distance_between_index_and_its_knuckle >= 150:
                pyautogui.leftClick()


            previous_x = index_finger_tracing_x
            previous_y = index_finger_tracing_y
